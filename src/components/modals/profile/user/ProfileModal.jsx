...
const ProfileModal = ({ onClose }) => {
  ...
  const [url, setUrl] = useState(
    "http://localhost:8080/api/v1/users/get-user-detail"
  );
  ...
  const { data, isLoading, isError } = useFetchUser(url, option);

  const { data1, isLoading1, isError1 } = useFetchPut(
    url1,
  ...
  const handleOnSubmit = () => {
    ...
    setUrl1("http://localhost:8080/api/v1/users/edit-user-detail");
  ...
  return (
    <div className="fixed flex w-full item-center justify-center h-full">
      <div
...
const FormSubmit = ({ fullNameRef, emailRef, phoneNumberRef, data }) => {
  return (
    <>
